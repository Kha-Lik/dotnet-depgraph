...
The **Manual layout** tab captures the active Explore projection into an independent, initially paused board. It provides exclusive rectangle/circle feature regions (including Unassigned), bounded node and region dragging, explicit local relaxation and group packing, pins, multi-node assignment, incident-edge hiding with temporary reveal, session undo/redo, truthful browser autosave status, and versioned `manual-layout.json` download/import. Returning to Explore restores its positions and viewport paused. Manual membership and hidden connections are presentation state only: they do not rewrite canonical edges, automatic communities, GraphML, or dependency metrics. Imported layouts must match both the graph topology and captured projection.
