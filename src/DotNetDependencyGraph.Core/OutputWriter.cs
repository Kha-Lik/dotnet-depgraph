...
public sealed record ReportOptions(IReadOnlyList<string> IncludePackages, IReadOnlyList<string> ExcludePackages,
    FilterMode FilterMode, int Seed = 42, bool Force = false, bool CollapseLocalPackages = false);

public static class OutputWriter
{
    private static readonly string[] KnownFiles = ["index.html", "graph.json", "diagnostics.json", "summary.md", "graph.graphml", "viewer.js", "viewer.css", "cytoscape.min.js", "THIRD-PARTY-NOTICES.txt"];
    ...
    public static void Write(string output, DependencyGraph raw, ReportOptions options, string viewerDirectory)
    {
        ...
        var strict = GraphFilter.Apply(raw, options.IncludePackages, options.ExcludePackages, FilterMode.Strict);
        var contract = GraphFilter.Apply(raw, options.IncludePackages, options.ExcludePackages, FilterMode.Contract);
