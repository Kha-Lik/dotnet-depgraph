  ...
  class ManualLayout {
    ...
    start(groupIds) {
      ...
      for (const groupId of requested) {
        ...
        if (!members.length) continue; const ids = new Set(members.map(n => n.id));
        const links = this.cy.edges().filter(edge => ids.has(edge.source().id()) && ids.has(edge.target().id()) && !muted.has(edge.source().id()) && !muted.has(edge.target().id())).map(edge => ({ source: edge.source().id(), target: edge.target().id() }));
        const region = board.groups[groupId], area = G.usable(region), center = area.shape === "circle" ? { x: area.cx, y: area.cy } : { x: (area.left + area.right) / 2, y: (area.top + area.bottom) / 2 };
        const simulation = d3.forceSimulation(members).stop().alpha(1).alphaDecay(.035).velocityDecay(.45)
          .force("link", d3.forceLink(links).id(d => d.id).distance(70).strength(.12))
          .force("charge", d3.forceManyBody().strength(-260).distanceMax(450))
          .force("collision", d3.forceCollide(d => d.radius + 5).iterations(4))
          .force("x", d3.forceX(center.x).strength(.025)).force("y", d3.forceY(center.y).strength(.025));
    ...
    tick() {
      ...
      for (const [groupId, run] of this.running) {
        ...
        run.simulation.tick();
        run.members.forEach(node => { const point = G.project(region, node, node.radius); if (point.x !== node.x) node.vx = 0; if (point.y !== node.y) node.vy = 0; node.x = point.x; node.y = point.y; });
        for (let pass = 0; pass < 4; pass++) for (let i = 0; i < run.members.length; i++) for (let j = i + 1; j < run.members.length; j++) {
          const a = run.members[i], b = run.members[j], minimum = a.radius + b.radius + 4, dx = b.x - a.x || .001, dy = b.y - a.y, distance = Math.hypot(dx, dy);
          if (distance >= minimum || (a.fx != null && b.fx != null)) continue; constrained = true; const push = (minimum - distance) / 2, ux = dx / distance, uy = dy / distance;
          if (a.fx == null) { a.x -= ux * push; a.y -= uy * push; Object.assign(a, G.project(region, a, a.radius)); }
          if (b.fx == null) { b.x += ux * push; b.y += uy * push; Object.assign(b, G.project(region, b, b.radius)); }
        }
        run.members.forEach(node => Object.assign(board.placements[node.id], { x: node.x, y: node.y }));
      }
      this.onPaint();
