...
public sealed class IntegrationTests
{
    ...
    public void WritesOfflineReportWithEscapedEmbeddedDataAndControls()
    {
        ...
        try
        {
            ...
            var summary = File.ReadAllText(Path.Combine(temp, "summary.md")); Assert.Contains("## Viewing the report", summary); Assert.Contains("## Viewer quick guide", summary); Assert.Contains("## Generated analysis", summary); Assert.Contains("Raw graph: 1 nodes, 0 edges", summary); Assert.Contains("## Detected communities", summary); Assert.Contains("| Community | Stable key | Nodes | Projects | Packages | Runnable | Representative nodes |", summary); Assert.Contains("&lt;/script&gt;&lt;script&gt;alert(1)&lt;/script&gt;", summary); Assert.DoesNotContain("{{", summary);
