...
public sealed class IntegrationTests
{
    ...
    public void WritesOfflineReportWithEscapedEmbeddedDataAndControls()
    {
        ...
        try
        {
            ...
            foreach (var file in new[] { "index.html", "graph.json", "diagnostics.json", "summary.md", "graph.graphml", "icons.js", "viewer.js", "viewer.css", "manual-geometry.js", "manual-state.js", "manual-storage.js", "manual-layout.js", "manual-view.js", "coloris.min.js", "coloris.min.css", "cytoscape.min.js", "d3-dispatch.min.js", "d3-quadtree.min.js", "d3-timer.min.js", "d3-force.min.js", "THIRD-PARTY-NOTICES.txt" }) Assert.True(File.Exists(Path.Combine(temp, file)), file);
            var html = File.ReadAllText(Path.Combine(temp, "index.html")); Assert.DoesNotContain("</script><script>alert(1)</script>", html); Assert.Contains("id=\"search\"", html); Assert.Contains("id=\"manual-search\"", html); Assert.Contains("id=\"hops\"", html); Assert.Contains("id=\"component\"", html); Assert.Contains("id=\"repulsion\"", html); Assert.DoesNotContain("src=\"http", html, StringComparison.OrdinalIgnoreCase);
            ...
            Assert.Contains("id=\"size-metric\"", html); Assert.Contains("id=\"runnable-min\"", html); Assert.Contains("showCommunityMap", viewer); Assert.Contains("state.selectionOrder", viewer); Assert.Contains("rememberNodeSelection(e.target.id())", viewer);
            Assert.Contains("id=\"tab-manual\"", html); Assert.Contains("id=\"manual-regions\"", html); Assert.Contains("manual-geometry.js", html); Assert.Contains("manual-layout.js", html); Assert.Contains("coloris.min.js", html); Assert.Contains("coloris.min.css", html); Assert.Contains("projectionFingerprint", viewer);
            var manualStorage = File.ReadAllText(Path.Combine(temp, "manual-storage.js")); Assert.Contains("dotnet-depgraph.manual-layout.v1", manualStorage); Assert.Contains("region-relative", manualStorage); Assert.Contains("graphFingerprint", manualStorage);
            Assert.Contains("Coloris 0.25.0", File.ReadAllText(Path.Combine(temp, "THIRD-PARTY-NOTICES.txt")));
